...
<EyeBrown label="Ethers Configuration" />;
...
<Alert
  ...
  icon={<IconInfoCircle />}
>
  Only chains that are configured in your project's Dynamic dashboard will be supported. Make sure
  to add the desired chains in your Dynamic dashboard before using them with Ethers.js.
</Alert>
...
```tsx
import { useSophonContext, isEthereumWallet, isZKsyncConnector } from "@sophon-labs/react";
