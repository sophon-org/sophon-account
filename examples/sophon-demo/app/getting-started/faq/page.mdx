...
function ChainSwitcher() {
  ...
  return <button onClick={() => switchChain("sophon")}>Switch to Sophon</button>;
}
```
...
Yes, the SDK undergoes regular security audits. You can find the latest audit reports in our [Security Guidelines](/getting-started/secutiry).
