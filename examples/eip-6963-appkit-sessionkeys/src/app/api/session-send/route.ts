...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sessionConfig) {
      return new Response(JSON.stringify({ error: "Session config not found" }), { status: 404 });
    }

    const revivedConfig = reviveBigInts(sessionConfig.sessionConfig);
    const sessionClient = getViemSessionClient(revivedConfig, from, PRIVATE_KEY, chain);
